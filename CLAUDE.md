...
Applies to any agent-assisted contribution to this repo. See [MASTER_PLAN.md](MASTER_PLAN.md) for phases/tasks and [DEVELOPER_README.md](DEVELOPER_README.md) for architecture and contracts.

* **One component + its tests per task.** Never bundle two components in one change. Task boundaries are the checkboxes in MASTER_PLAN.md §5.
...
* **`make validate` must be green before push.** The pre-push hook and GitHub Actions run the identical command — see MASTER_PLAN.md §3.
