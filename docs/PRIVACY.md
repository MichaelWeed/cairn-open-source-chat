...
| Safe telemetry | Strict bounded counters and duration/total-token histograms derived only from source-free copies of finalized request-accounting summaries and readiness reports. No request, session, content, citation, path, endpoint, credential, exception, corpus identity, or monetary value is present. The production sink is stateless and performs no I/O. | One request or readiness evaluation only; no production persistence/exporter is included. |
