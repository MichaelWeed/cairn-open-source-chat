...
| Structured logs | stdout, JSON. A mechanical typed allowlist permits only fixed event names, normalized error codes, retry state/attempt, and bounded startup counts. External or malformed records become a fixed `external_log`; messages, paths, exceptions, and arbitrary extras are never serialized. | Whatever your log aggregation/host retains. |
| Provider usage metadata | Bounded token counts, provider/model identifiers, attempt number, optional service tier, and the terminal private request summary are normalized in memory. They contain no chat content or session identifier, are handed to one no-op lexical owner after stream cleanup, and are excluded from public SSE, persistence, and logs. | Request lifetime only. |
| Readiness evidence | Fixed internal states for deployment mode, database, selected retrieval route, corpus, provider/model, embedding, exact corpus, and budget requirement. Raw responses and exceptions are discarded; public `/readyz` exposes only its existing status and three booleans. | One readiness request only. |
...
| Safe telemetry | Strict bounded counters and histograms from source-free accounting/readiness models, request timing and per-app concurrency. Corpus labels describe only local-active, exact-version, or unknown route state; no raw corpus ID/version is retained. Attempt cost carries only a validated amount/currency or explicit unknown state. Host handoff/resolution signals are explicit fixed enums. No request/session identity, content, citation, path, endpoint, credential, exception, price source, or customer identity is present. The default sink performs no I/O. | One request or readiness evaluation, with one per-app concurrency counter and host-port failure fuse; no production persistence/exporter is included. |
| Deployment-wide admission state | Domain-separated HMAC-SHA-256 digests of canonical client IP and session identity, bounded rate counters, fenced tickets/leases, operation receipts, and Decimal UTC-hour/day budget totals. Raw IPs, sessions, headers, prompts, provider text, and exceptions are excluded. | Exact configured rate, ticket, lease, budget-window, and receipt expiries; the included store is development/test only. |
