...
function clientScript(scenario) {
  return `<script nonce="${NONCE}">(async()=>{try{
    ...
    const skip=document.querySelector(".skip-link");if(scenario==="forced"){window.__cairnForcedReady=true;await waitFor(()=>window.__cairnForcedActivated===true);}else await waitFor(()=>{skip.focus();return document.activeElement===skip&&getComputedStyle(skip).transform==="none";});const skipStyle=getComputedStyle(skip),skipBoxFocused=rect(skip),skipFocusColor=skipStyle.outlineColor,skipFocusStyle=skipStyle.outlineStyle,skipVisible=(skipStyle.transform==="none"||skipBoxFocused.top>=0)&&document.activeElement===skip;
