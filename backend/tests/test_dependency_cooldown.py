...
def _configure_widget_release(
    ...
    monkeypatch.setattr(cooldown, "backend_packages", lambda: [])
    monkeypatch.setattr(cooldown, "widget_packages", lambda: [(name, version)])
    monkeypatch.setattr(cooldown, "npm_release_date", lambda _name, _version: released)


def test_early_release_approval_matches_only_the_exact_pypdf_pair(cooldown: ModuleType) -> None:
    assert cooldown.is_early_release_approved("pypi", "pypdf", "6.18.0")
    assert not cooldown.is_early_release_approved("pypi", "pypdf", "6.18.1")
    assert not cooldown.is_early_release_approved("pypi", "another-package", "6.18.0")


def test_early_release_approval_matches_only_the_exact_js_yaml_pair(cooldown: ModuleType) -> None:
    assert cooldown.is_early_release_approved("npm", "js-yaml", "4.3.2")
    assert not cooldown.is_early_release_approved("npm", "js-yaml", "4.3.1")
    assert not cooldown.is_early_release_approved("pypi", "js-yaml", "4.3.2")
...
def test_approved_js_yaml_release_passes_inside_cooldown_window(
    monkeypatch: pytest.MonkeyPatch, cooldown: ModuleType
) -> None:
    _configure_widget_release(
        monkeypatch, cooldown, "js-yaml", "4.3.2", datetime.now(UTC) - timedelta(days=13)
    )
...
def test_ordinary_release_age_passes_at_or_after_cooldown_window(
    ...
    def fail_if_approval_is_checked(_ecosystem: str, _name: str, _version: str) -> bool:
